...
//	private int nextSource;
	...
	public Audio getOggStream(String ref) throws IOException {
		if (!soundWorks) {
			throw new IOException("Sound isn't working right now");
		}
	...
	public Audio getOgg(String ref) throws IOException {
		return getOgg(ref, getClass().getResourceAsStream(ref));
	}
	...
	public Audio getOgg(String ref, InputStream in) throws IOException {
		...
		if (loaded.get(ref) != null) {
			...
			try {
				...
				OggData ogg = null;
				if(ref.endsWith(".buffer")){
					SerializableSoundWrapper rawSound = (SerializableSoundWrapper) ObjectLoading.loadObject(Game.getFile(ref));
					ogg = rawSound.toOggData();
				} else {
